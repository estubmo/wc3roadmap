...
**Domain:** Interactive learning-roadmap web app (WC3 / RTS domain)
**Researched:** 2026-06-28
**Confidence:** MEDIUM (all library facts verified via npm/official docs; w3champions API details inferred from open-source code — no official third-party documentation exists)
...
This is the most important feasibility question. Here is the verified picture:

### What Battle.net OAuth provides

Battle.net has a fully documented OAuth 2.0 flow for third-party apps (developer portal: `community.developer.battle.net`). Register an app, get `clientId` + `clientSecret`, configure redirect URI.

Without any scopes, OAuth returns: **Blizzard account ID** + **BattleTag** (e.g. `Player#1234`).

There is **no dedicated WC3 scope** — `wow.profile` and `sc2.profile` exist but not `wc3.profile`. This is fine: BattleTag is the only identifier needed.

### What w3champions provides

w3champions uses Battle.net OAuth internally for their own authentication. Their `identification-service` (open-source on GitHub) issues JWTs after Battle.net login.

For **this project**, w3champions identity is not a separate OAuth step. The flow is:

```
1. User clicks "Login with Battle.net"
2. Battle.net OAuth → returns BattleTag
3. App stores BattleTag in our DB (via better-auth session)
4. App calls w3champions API using BattleTag to fetch ladder stats
```

### w3champions API feasibility (MEDIUM confidence — unverified rate limits)

The w3champions frontend (`w3champions.com`) is open source and calls `api.w3champions.com` for all player data. The API is publicly accessible (no auth needed for read-only stats). It exposes player MMR, match history, win rates by race, hero statistics. However:
...
Mitigation: wrap all w3champions API calls in TanStack Query with aggressive `staleTime` (e.g. 5 minutes per player profile). Cache server-side where possible. Budget for the API to become unavailable or change — graceful degradation to manual-only progress tracking.

### better-auth configuration for Battle.net

Use the `genericOAuth` plugin to configure Battle.net:
  ...
  plugins: [
    genericOAuth({
      config: [{
        ...
        scopes: [], // no scopes needed for BattleTag + account ID
      }]
    })
  ]
})
...
The handler mounts at `/api/auth/$` in TanStack Start.

**Confidence on this flow: MEDIUM.** The OAuth endpoints are documented. The genericOAuth plugin is documented. The specific Battle.net + better-auth combination has no known community examples yet — budget for 1-2 days of integration work to resolve edge cases (region-specific OAuth hosts: `us.battle.net`, `eu.battle.net`, etc.).

---

## Graph Library Decision: @xyflow/react

**Recommendation: `@xyflow/react` (React Flow v12). Do not use Cytoscape.js.**
...
At the WC3 roadmap scale (50–200 nodes maximum), Cytoscape's performance advantage for very large graphs is irrelevant. React Flow's native React model means node content, mastery badges, pathway highlights, and tooltips are all standard React + Tailwind — no canvas API or imperative mutations needed.
...
**Recommendation: Vercel (primary), with Cloudflare Workers as a stretch goal.**
...
For Cloudflare Workers: the TanStack Start + Neon + Drizzle stack has a documented working example (isaacfei.com blog post). The `nodejs_compat` flag is required. better-auth requires testing in workerd runtime — not confirmed working there as of June 2026.

**Start with Vercel. Migrate to Cloudflare if cold starts become a UX problem.**
...
# Content pipeline
npm install @content-collections/core @content-collections/mdx
...
*Stack research for: WC3 Learning Roadmap — TanStack Start ecosystem*
*Researched: 2026-06-28*
