...
const WPMFile = class {
  ...
  // cacheJson is the shape written by `write()`: { grid: [[...rows...]] }.
  // The grid is already fully-formed cells; processTiles is not re-run.
  static fromCache (cacheJson, mapData) {
    const inst = Object.create(WPMFile.prototype);
    inst.grid = cacheJson.grid;
    inst.mapHeight = inst.grid.length / 4;
    inst.mapWidth = (inst.grid[0] ? inst.grid[0].length : 0) / 4;
    return inst;
  }
